 ...
 *
 * The client carries its own external store (`client.store`), so the hook no
 * longer wires seven `onXChange` callbacks into React state — it just reads
 * `client.store` through `useSyncExternalStore`. Only the genuine side-effect
 * callbacks remain, and they read through `optionsRef` so changing them between
 * renders never recreates the client.
 */
...
>(
  ...
  // The client *is* the store. `client.store.subscribe` / `getSnapshot` are
  // stable per client, so there is no re-subscription churn.
