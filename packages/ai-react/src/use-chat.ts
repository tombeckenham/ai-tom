...
/**
 * Build the `ChatClient` and its bound action set.
 *
 * The client's reactive state lives in a `@tanstack/store` Store
 * (`client.store`), so the hook reads it with `useStore` — no `onXChange`
 * callbacks into React state. Only the genuine side-effect callbacks remain,
 * and they read through `optionsRef` so changing them between renders never
...
): { client: ChatClient<TTools, TContext>; actions: ChatActions<TTools> } {
...
>(
  ...
  // The client's stable callbacks read the latest options through this ref, so
  // changing side-effect handlers or structural options never recreates it.
  // Synced during render so it is current before any callback fires.
  const optionsRef = useRef(options)
  optionsRef.current = options

  // One client per `clientId`. Connection/body/context changes flow through
  // `updateOptions` below rather than a remount, so the client is recreated
  // only when its identity (`clientId`) actually changes.
  const { client, actions } = useMemo(
    () => createChatClient<TTools, TSchema, TContext>(clientId, optionsRef),
    [clientId],
  )

  // The client's state is a @tanstack/store Store; useStore subscribes and
  // re-renders on change. A selector (`useStore(client.store, s => s.messages)`)
  // could scope re-renders per field — kept whole here to match the public API.
